...
void loop() {
  lcd.createChar(0,RIO);
  lcd.createChar(1,LAGO);
  lcd.createChar(2,MAQ);
  lcd.createChar(3,RAYO);
  lcd.createChar(4,SGA);
  lcd.createChar(5,RAD0);
  lcd.createChar(6,RAD1);
  lcd.createChar(7,RAD2);
  ...
  if (WiFi.status() == WL_CONNECTED) {
   ...
   CHECK_OK.end();  
    ...
    /* Start Drawing crap */
    UPS.begin(client, "http://XXX.XXX.XXX.XXX:XXXX/api/last_ups_data");  
    ...
    UPS.end();
    /* second part */
    SADI.begin(client, "http://XXX.XXX.XXX.XXX:XXXX/api/last_sadi");  // URL actualizada
    ...
    if (httpCode2 == HTTP_CODE_OK) {
      ...
      lcd.print(demHoy);
      lcd.print(" MW");
      lcd.write(byte(3));// Limpia caracteres residuales
    }
    SADI.end();
    /* Salto Grande */
    SG.begin(client, "http://XXX.XXX.XXX.XXX:XXXX/api/last_sg");  // URL para last_sg
    int httpCode3 = SG.GET();
    Serial.println(httpCode3);
    if (httpCode3 == HTTP_CODE_OK) {  /* Si hay Internet */ 
  ...
  for (int i = 0; i <= 15; i++) {
        lcd.setCursor(17, 3); lcd.print(" "); delay(250);
        lcd.setCursor(17, 3); lcd.write(byte(5));  delay(250);
        lcd.setCursor(17, 3); lcd.write(byte(6));  delay(250);
        lcd.setCursor(17, 3); lcd.write(byte(7));  delay(250);
        }
